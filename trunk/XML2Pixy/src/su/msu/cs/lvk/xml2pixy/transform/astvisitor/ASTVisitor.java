...
public class ASTVisitor {
    ...
    public void visit(Node node, String currentFile1, int lineno, String module) throws VisitorException {
        ...
        if (Character.isUpperCase(node.getJdomElement().getName().charAt(0))) {
            if (currentFile.length() > 4 &&
                    ".xml".equals(currentFile.substring(currentFile.length() - 4, currentFile.length()))) {
    ...
    protected ParseNode makeBinary(Node node, int lineno) throws VisitorException {
        Node leftNode = getFirstChild(node, "left"),
                rightNode = getFirstChild(node, "right");
        Node leftChild = getFirstChild(leftNode, null),
                rightChild = getFirstChild(rightNode, null);
    ...
    protected ParseNode getBinaryOperation(String name, int lineno) throws VisitorException {
        ...
        if (name.equals("Add")) {
            ...
            throw new VisitorException("Unknown binary operation \"" + name + "\" at " +
                    helper.getCurrentFile() + ":" + lineno);
        }
    ...
    protected ParseNode getUnaryOperation(String name, int lineno) throws VisitorException {
        ...
        if (name.equals("UnaryAdd")) {
            ...
            throw new VisitorException("Unknown unary operation \"" + name + "\" at " +
                    helper.getCurrentFile() + ":" + lineno);
        }
    ...
    protected ParseNode makeExprInBraces(ParseNode expression) {
        ...
        return helper.create(PhpSymbols.expr,
                helper.create(PhpSymbols.expr_without_variable,
                        helper.create(PhpSymbols.T_OPEN_BRACES, "(", linenoLeft),
                        makeExpr(expression),
                        helper.create(PhpSymbols.T_CLOSE_BRACES, ")", linenoRight)
                ));
    }
    ...
    protected ParseNode makeCompare(String operation, int lineno) {
        ...
        if (Utils.isBlank(operation))
            ...
            return null; // maybe "in" and "not in" operator... wrong operations will be handled later
        }
    ...
    protected ParseNode makeInnerStatementListInBraces(ParseNode stmt, int lineno) {
        ...
        return helper.create(PhpSymbols.statement,
                helper.create(PhpSymbols.unticked_statement,
                        helper.create(PhpSymbols.T_OPEN_CURLY_BRACES, "{", lineno),
                        stmt,
                        helper.create(PhpSymbols.T_CLOSE_CURLY_BRACES, "}", linenoRight)));
    }
    ...
    protected ParseNode makeExpr(ParseNode cvar) {
        ...
        if (cvar.getSymbol() == PhpSymbols.cvar) {
            return helper.create(PhpSymbols.expr,
                    helper.create(PhpSymbols.r_cvar, cvar));
    ...
    protected ParseNode makeReferenceVariable(ParseNode tString) {
        if (tString.isToken()) {
            ...
            return helper.create(PhpSymbols.reference_variable,
                    helper.create(PhpSymbols.compound_variable,
                            helper.create(PhpSymbols.T_VARIABLE, lexeme, tString.getLineno())));
        }
        return tString;
    }
    ...
    protected ParseNode makeCvar(ParseNode reference_variable) {
        ...
        if (reference_variable.getSymbol() == PhpSymbols.reference_variable) {
            return helper.create(PhpSymbols.cvar,
                    helper.create(PhpSymbols.cvar_without_objects, reference_variable));
        } else {
    ...
    protected ParseNode removeSemicolon(ParseNode node) {
        if (node.getSymbol() != PhpSymbols.top_statement_list) return node;
        else {
            return null;
        }
    ...
    protected ParseNode renderStringConstant(int lineno, String value) {
        return helper.create(PhpSymbols.expr,
                helper.create(PhpSymbols.expr_without_variable,
                        helper.create(PhpSymbols.scalar,
                                helper.create(PhpSymbols.common_scalar,
                                        helper.create(PhpSymbols.T_CONSTANT_ENCAPSED_STRING,
                                                '"' + value + '"', lineno)))));
    }
    ...
    protected ParseNode makeFunctionCall(String function, ParseNode[] args, ParseNode firstArg, int lineno) {
        ...
        ParseNode function_call = helper.create(PhpSymbols.function_call,
                helper.create(PhpSymbols.T_STRING, function, lineno),
                helper.create(PhpSymbols.T_OPEN_BRACES, "(", lineno),
    ...
    protected ParseNode addFirstTopStatement(ParseNode where, ParseNode what) {
        ...
        if (where.getSymbol() != PhpSymbols.top_statement_list ||
                (what.getSymbol() != PhpSymbols.statement &&
                        what.getSymbol() != PhpSymbols.declaration_statement)) {
            return where;
        }

        // If empty top statement
        ...
        if (where.getNumChildren() == 1) {
            ...
            // where points to T_EPSILON
            where = where.getParent().getParent();
            // where points to deepest top_statement_list with 2 children
    ...
    protected ParseNode addLastTopStatement(ParseNode where, ParseNode what) {
        ...
        // If empty top statement
        return helper.create(PhpSymbols.top_statement_list,
                where,
                helper.create(PhpSymbols.top_statement,
                        what));
    }
    ...
    protected ParseNode[] getArgs(String functionName, Node argsNode, int lineno) throws VisitorException {
        ...
        // if unknown just make argslist from argsNode
        ...
        // else do everything
        if (fun.isFunction() && fun.getArgs() != null) { // function, but not builtin function
        ...
        for (Node child : nodes) {
            if (child.getJdomElement().getName().equals("Keyword")) {
                ...
                if (index < 0) {
                    log.warn("Unresolved argument " + arg + " in function " + fun.getModule() + "." + fun.getName() +
                            " (" + helper.getCurrentFile() + ":" + lineno + ")");
                ...
                if (keyword) throw new VisitorException("Syntax error: non-keyword arg after keyword args (" +
                        child.getParseNode().getFileName() + ":" + child.getParseNode().getLinenoLeft());
                args[i++] = child.getParseNode();
            }

            // make factIndex first null argnum
            factIndex = 0;
        ...
        for (i = 0; i < args.length; i++) {
            if (args[i] == null) {
                ...
                if (defaultValue == null) {
                    log.warn("Couldn't find value for argument #" + i + " of function "
                            + fun.getModule() + "." + fun.getName());
                } else {
    ...
    protected ParseNode makeNonStatic(ParseNode node, int lineno) {
        ...
        switch (node.getChild(0).getSymbol()) {
            ...
            case PhpSymbols.T_STRING:
                result = helper.create(PhpSymbols.expr_without_variable,
                        helper.create(PhpSymbols.scalar, duplicate(node.getChild(0))));
                break;
            case PhpSymbols.T_PLUS:
            case PhpSymbols.T_MINUS:
                result = helper.create(PhpSymbols.expr_without_variable,
                        duplicate(node.getChild(0)),
                        helper.create(PhpSymbols.expr, makeNonStatic(node.getChild(1), lineno)));
                break;
            case PhpSymbols.T_ARRAY:
                result = helper.create(PhpSymbols.expr_without_variable,
                        helper.create(PhpSymbols.T_ARRAY, "array", lineno),
                        helper.create(PhpSymbols.T_OPEN_BRACES, "(", lineno),
                        makeNonStaticArrayPairList(node.getChild(2), lineno),
                        helper.create(PhpSymbols.T_CLOSE_BRACES, ")", lineno));
                break;
            default:
                result = null;
        }
    ...
    protected ParseNode makeNonStaticArrayPairList(ParseNode original, int lineno) {
        if (original.getNumChildren() < 2) {
            ...
            return helper.create(PhpSymbols.array_pair_list,
                    makeNonStaticNonEmptyArrayPairList(original.getChild(0), lineno),
                    duplicate(original.getChild(1))/*possible comma*/);
        }
    ...
    protected ParseNode makeNonStaticNonEmptyArrayPairList(ParseNode original, int lineno) {
        ...
        if (original.getNumChildren() == 1) {
            ...
            helper.addChild(non_empty_array_pair_list, makeNonStaticNonEmptyArrayPairList(original.getChild(0), lineno))
                    .addChild(helper.create(PhpSymbols.T_COMMA, ",", lineno))
                    .addChild(makeExpr(original.getChild(2)))
                    .addChild(helper.create(PhpSymbols.T_DOUBLE_ARROW, "=>", lineno))
                    .addChild(makeExpr(original.getChild(4)));
        } else {
            switch (original.getChild(0).getSymbol()) {
                ...
                case PhpSymbols.static_scalar:
                    helper.addChild(non_empty_array_pair_list, makeExpr(original.getChild(0)))
                            .addChild(helper.create(PhpSymbols.T_DOUBLE_ARROW, "=>", lineno))
                            .addChild(makeExpr(original.getChild(2)));
                    break;
                default:
            ...
            for (ParseNode child : args) {
                if (non_empty_function_call_parameter_list == null) {
                    ...
                    non_empty_function_call_parameter_list = helper.create(PhpSymbols.non_empty_function_call_parameter_list,
                            ...
                            helper.create(PhpSymbols.T_COMMA, ",", lineno));
                }
    ...
    protected ParseNode makeStatementFromExpr(ParseNode expr, int lineno) {
        if (expr.getSymbol() != PhpSymbols.expr) {
            ...
            return helper.create(PhpSymbols.statement,
                    helper.create(PhpSymbols.unticked_statement,
                            expr,
                            helper.create(PhpSymbols.T_SEMICOLON, ";", lineno)));
        }
    }
    ...
    protected ParseNode makeStaticScalar(Node node) {
        ...
        if (nodeName.equals("Name")) {
            ...
            if (!name.equals("None")) {
                ...
                static_scalar.addChild(
                        helper.create(PhpSymbols.T_STRING, "null", getLineno(node.getJdomElement())));
            }
            ...
            if (arg == null) {
                log.warn("WARNING: got not prepared default value in function def (" +
                        helper.getCurrentFile() + ":" + getLineno(node.getJdomElement()) + ")");
                return null;
            }
            helper.addChild(static_scalar, helper.create(PhpSymbols.T_PLUS, "+", getLineno(node.getJdomElement())))
                    .addChild(arg);
            ...
            helper.addChild(static_scalar, helper.create(PhpSymbols.T_MINUS, "-", getLineno(node.getJdomElement())))
                    .addChild(arg);
            ...
            log.warn("WARNING: doesn't support default value with node " + nodeName + " (" +
                    helper.getCurrentFile() + ":" + getLineno(node.getJdomElement()) + ")");
            return null;
        }
    ...
    protected ParseNode makeStaticArray(Node node) {
        ...
        if (nodes.getChildren().isEmpty()) {
            ...
            helper.addChild(static_array_pair_list, non_empty_static_array_pair_list)
                    .addChild(possible_comma);
        }

        return helper.create(PhpSymbols.static_scalar,
                helper.create(PhpSymbols.T_ARRAY, "array", lineno),
                helper.create(PhpSymbols.T_OPEN_BRACES, "(", lineno),
    ...
    protected ParseNode makeStaticDictionary(Node node) {
        ...
        if (items.getChildren().isEmpty()) {
            ...
            for (int i = 0; i < items.getChildren().size(); i += 2) {
                ...
                helper.addChild(non_empty_static_array_pair_list, keypn).
                        addChild(helper.create(PhpSymbols.T_DOUBLE_ARROW, "=>", lineno))
                        .addChild(valuepn);
            }
            helper.addChild(static_array_pair_list, non_empty_static_array_pair_list)
