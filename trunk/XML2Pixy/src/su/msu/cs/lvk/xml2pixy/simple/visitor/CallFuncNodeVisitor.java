        ...
        if (callee instanceof GetattrNode) {
            ...
            if (obj.getClass() == NameNode.class) {
                ...
                if (symbol != null && symbol.isGenerator()) {
                    ...
                    callFunc.setNode(new NameNode(ProcessingUtils.concatMangled(fullGeneratorName, ProcessingUtils.GENERATOR_NEXT))
                            .copyLocation(node));
                    return;
                } 
            }
            callFunc.getArgs().add(0, obj);
            callFunc.setNode(new NameNode(ProcessingUtils.getMethodFullName(calleeGetattr.getAttrName(), callFunc.getArgs().size()))
                    .copyLocation(node));
            ...
            if (symbol != null) {
                if (symbol.isClass()) {
                    name.setName(ProcessingUtils.concatMangled(PyClass.CONSTRUCTOR_NAME,
                            ProcessingUtils.getFullName((ClassNode) symbol.getSource())));
