...
def handler(req, args):
	...
	con = MySQLdb.connect(  host = settings.database_settings["host"],
		...
		row = cur.fetchone()
		contents = "<h2>Delete news</h2>" + os.linesep
		contents = contents + "<form action=\"dodeletenews.html\" method=\"POST\">"
		...
		while row and i < NewsToQuery:
			...
			contents = contents + "<input type=\"checkbox\" name=\"%s\">" % (row[5], ) + os.linesep
		...
		contents = contents + "<td colspan=\"2\" align=\"left\">" + os.linesep
		if skip > 0:
			contents = contents + "<a href=\"deletenews.html?skip=%s\">Previous %s news...</a> <br/>" % (str(skip-1), str(settings.general_settings["max_news_per_page"])) + os.linesep
		
		contents = contents + "</td>" + os.linesep
		
		contents = contents + "<td align=\"right\">" + os.linesep
		if int(cur.rowcount) == NewsToQuery:
			contents = contents + "<a href=\"deletenews.html?skip=%s\">Next %s news...</a> <br/>" % (str(skip+1), str(settings.general_settings["max_news_per_page"])) + os.linesep
		...
		contents = contents + "<input type=\"submit\" value=\"Delete\">"  + os.linesep
		contents = contents + "</td>" + os.linesep
