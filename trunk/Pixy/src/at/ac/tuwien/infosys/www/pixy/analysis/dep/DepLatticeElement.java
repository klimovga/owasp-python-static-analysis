...
//  ********************************************************************************
    ...
    private static DepSet getDefaultDep(TacPlace place) {
        ...
        DepSet depSet = DepLatticeElement.DEFAULT.getPlaceToDep().get(place);
        return (depSet != null) ? depSet : DepSet.create(new HashSet<Dep>());
    }
...
//  ********************************************************************************
    ...
    private DepSet getDefaultArrayLabel(Variable var) {
        DepSet depSet = DEFAULT.arrayLabels.get(var);
        return (depSet != null) ? depSet : DepSet.create(new HashSet<Dep>());
    }
...
// setWholeTree ********************************************************************
    ...
    // sets the dep for all literal array elements in the
    // tree sepcified by the given root, INCLUDING THE ROOT
    private void propagateRightElementTree(Variable root, DepSet depSet, Set<Dep> newSet) {
...
//  ********************************************************************************
    ...
    public void assign(Variable left, 
        ...
        case 2:
        {
            ...
            this.setWholeTree(left, oldDep);
//            this.propagagateRightElementTree(left, oldDep);
            break;
        }
...
//  handleReturnValue **************************************************************
    public void handleElementsDeps(Variable left, DepSet deps) {
        this.lubArrayLabel(left, deps);
        // increases false positives rate, but keeps conservativeness under arrays!
        this.setDep(left, lub(getDep(left), deps));
    }
