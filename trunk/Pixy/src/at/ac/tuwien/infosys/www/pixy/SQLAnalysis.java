...
public class SQLAnalysis extends DepClient {
    ...
    protected static Logger log = Logger.getLogger(SQLAnalysis.class.getName());
...
//  ********************************************************************************
    ...
    public List<Integer> detectVulns() {
        ...
        log.info("Performing SQL Analysis");
        ...
        log.info("Performing SQL Analysis: finished");
        ...
        for (Sink sink : sinks) {
            ...
            for (DepGraph depGraph : depGraphs) {
                ...
                if (!tainted) {
                    ...
                    log.info("Vulnurablity in file " + cfgNode.getLoc());
                    log.info("See graphs: sql" + graphcount);

                    
                    String vulnMessage = "Vulnerability detected in '" + cfgNode.getLoc() + "'";
                    vulnMessage += ". See graph: sql" + graphcount + " for details";
                ...
                if (tainted) {
                    ...
                    if (!dangerousUninit.isEmpty()) {
                        if (dangerousUninit.values().contains(InitialTaint.ALWAYS)) {
                            log.debug("- unconditional");
                        } else {
                            log.debug("- conditional on register_globals=on");
                        }
        ...
        if (MyOptions.optionV) {
            log.info("Total Graph Count: " + graphcount);
        }
...
//  ********************************************************************************
    ...
    private Automaton makeAutoForOp(DepGraphOpNode node, Map<DepGraphNode,Automaton> deco,
        ...
        // CATCH-ALL ********************************************
            ...
            log.warn("Unmodeled builtin function (SQL): " + opName);
...
//  ********************************************************************************
    ...
    void dumpDotAuto(Automaton auto, String graphName, String path) {
        ...
        try {
            ...
            log.error("Error writing dot file '" + path + "/" + filename + "'", e);
            return;
        }
