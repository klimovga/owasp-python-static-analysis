...
class AnalysisVisitor(ASTVisitor):
    ...
    def dispatch(self, node, *args):
        ...
        AnalysisVisitor.call_stack.append((self.filename, node))

        try:
            ASTVisitor.dispatch(self, node, *args)
            AnalysisVisitor.call_stack.pop()
            ...
            stack_copy = copy.copy(AnalysisVisitor.call_stack)
            stack_copy.reverse()

            for (f, n) in stack_copy:
                if hasattr(n, 'filename'):
                    print str(n.filename) + ":" + str(n.lineno),
                else:
                    print str(f) + ":" + str(n.lineno),
                print '(class ' + n.__class__.__name__ + ')'
                            ...
                            if methods != []:
                                for method in methods:
                                    ...
                                    try:
                                        ...
                                        print 'InvalidTargetError'
                                        undesirable_targets.append(exc)
                                    except NoTargetsError, exc:
#                                        print 'NoTargetsError'
#                                        print "callfunc: " + str(callfunc), "method: " + str(method), "args: " + str(args)
#                                        print "star_args: " + str(callfunc.star_args), "dstar_args: " + str(callfunc.dstar_args)
                                        undesirable_targets.append(exc)
...
#            print str(context_items)
    ...
    def visitGetattr(self, getattr, handler):
        ...
        for obj in lobj(getattr.expr, strict=1):
            ...
            if not attributes:
                if not hasattr(obj, '_class'):
                    print str(getattr.expr), str(obj), dir(obj)
...
class AnalysisSession:
    ...
    def process_builtins(self):
        ...
        if self.builtins is None:
            ...
            if not pythonPath is None:
                filename = os.path.join(os.getenv("PYTHONPATH"), filename)
    ...
    def process_name(self, module_name):
        ...
        testsdir = os.getenv("PYTHONPATH") + os.sep + "tests"
        libdir = os.getenv("PYTHONPATH") + os.sep + "lib"
        current = os.path.split(self.current_filenames[-1])[0]
        for d in [testsdir, libdir, base, current]:
            modulepath = os.path.join(d, module_name)

            if os.path.isdir(modulepath):
                pathname = os.path.join(modulepath, "__init__" + os.path.extsep + "py")
            else:
                pathname = modulepath + os.path.extsep + "py"


            if os.path.exists(pathname):

                if pathname in AnalysisSession.visitedModules.keys():
                    print "Second Module Import:", module_name, pathname
                    return AnalysisSession.visitedModules[pathname]
                else:
                    print "First Module Import:", module_name, pathname
                    try:
                        AnalysisSession.visitedModules[pathname] = compiler.parse("")
                        module = self.process_file(pathname)
                        AnalysisSession.visitedModules[pathname] = module
                        return module
                    except:
#                        traceback.print_stack()
                        raise

        print str([testsdir, libdir, base, current])
...
def process_file(filename, pypath):
