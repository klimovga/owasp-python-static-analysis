...
public class CallFuncNodeVisitor extends PythonNodeVisitor {
    ...
    public void visit(PythonNode node) throws VisitorException {
        ...
        if (callFunc.getNode() instanceof NameNode) {
            ...
            // if list.append() method
            if (callee.getName().startsWith(Method.DISPATCHER_PREFIX + ListNode.APPEND_METHOD) &&
                    !callFunc.getArgs().isEmpty()) {
                    ...
                    if (arg0.getClass() == NameNode.class) {
                        ...
                        if (name.getName().endsWith(ListComprehensionVisitor.LIST_COMP_RESULT)) {
                            ...
                            ParseNode refVar = helper.create(PhpSymbols.reference_variable,
                                    helper.create(PhpSymbols.reference_variable,
                                            helper.create(PhpSymbols.compound_variable,
                                                    helper.create(PhpSymbols.T_VARIABLE, "$" + name.getName(), node.getLineno()))),
                                    helper.create(PhpSymbols.T_OPEN_RECT_BRACES, "[", node.getLineno()),
                                    helper.create(PhpSymbols.dim_offset, makeEpsilon()),
                                    helper.create(PhpSymbols.T_CLOSE_RECT_BRACES, "]", node.getLineno())
                            );

                            // build statement
                            ParseNode stmt = helper.create(PhpSymbols.unticked_statement,
                                    helper.create(PhpSymbols.expr,
                                            helper.create(PhpSymbols.expr_without_variable,
                                                    makeCvar(refVar),
                                                    helper.create(PhpSymbols.T_ASSIGN, "=", node.getLineno()),
                                                    makeExpr(callFunc.getArgs().get(1).getPhpNode()))
                                    ),
            ...
            // if we have call to "eval"

            if (callFunc.getPhpNode() == null) {
                if (callee.getName().equals(FunctionNode.EVAL)) {
                    callFunc.setPhpNode(helper.create(PhpSymbols.expr,
                            helper.create(PhpSymbols.expr_without_variable,
                                    helper.create(PhpSymbols.internal_functions_in_yacc,
                                            helper.create(PhpSymbols.T_EVAL, "eval", callFunc.getLineno()),
                                            helper.create(PhpSymbols.T_OPEN_BRACES, "(", callFunc.getLineno()),
                                            makeExpr(callFunc.getArgs().get(0).getPhpNode()),
                                            helper.create(PhpSymbols.T_CLOSE_BRACES, ")", callFunc.getLineno())
                                    )
                            )
                    ));
                }
            ...
            if (callFunc.getPhpNode() == null) {
                ...
                for (PythonNode arg : callFunc.getArgs()) {
                    if (arg.getPhpNode() != null) {
                        ...
                        args.add(helper.create(PhpSymbols.expr_without_variable,
                                helper.create(PhpSymbols.scalar,
                                        helper.create(PhpSymbols.T_STRING, "null", arg.getLineno()))));
                    }
                }
