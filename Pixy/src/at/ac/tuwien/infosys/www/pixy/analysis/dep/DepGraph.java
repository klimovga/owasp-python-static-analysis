...
public class DepGraph {
    ...
    protected static Logger log = Logger.getLogger(DepGraph.class.getName());
...
//  *********************************************************************************
    ...
    private DepGraphNode makeDepGraph(TacPlace place, CfgNode current, TacFunction function, 
        ...
        log.debug("  visiting: " + current.getClass() + ", " + current.getOrigLineno() + ", " + place);
        ...
        */
        log.debug("in function : " + function.getName());
        log.debug("under contexts: " + contexts);
        ...
        log.debug("start going to nodes...");
        for (Dep dep: depSet.getDepSet()) {
            if (dep == Dep.UNINIT) {
                ...
                log.debug("getting used places for " + dep.getCfgNode().getOrigLineno());
    ...
    throws NotReachableException {
        ...
        if (enclosingX instanceof CfgNodeBasicBlock) {
            ...
            try {
                ...
                log.error("NullPointerException at " + cfgNode.getLoc() + " " + place, e);
                throw e;
            }
        }
...
//  ********************************************************************************
    ...
    private ContextSwitch switchContexts(TacFunction function, Set<Context> contexts, 
        ...
        if (current instanceof CfgNodeCallRet) {
            ...
            log.debug("jumping from caller to end of callee: " + function.getName() + " -> " + targetFunction.getName());
            targetContexts = new HashSet<Context>();
            for (Context c : contexts) {
                targetContexts.add(this.depAnalysis.getPropagationContext(callNode, c));
            }
            log.debug("target contexts: " + targetContexts);
        }
        ...
        // (i.e., from the callee's start to the call node)
            ...
            log.debug("jumping from start of callee to caller!");
            ...
            log.debug("caller: " + targetFunction.getName());
            targetContexts = new HashSet<Context>();
            for (Context c : contexts) {
...
//            }
        ...
        // the global variable was written, without warning
            ...
            log.debug("Unexpected function change: " + function.getName() + " -> " + Cfg.getFunction(targetNode).getName());
            targetFunction = Cfg.getFunction(targetNode);
            targetContexts = this.analysisInfo.getAnalysisNode(targetNode.getSpecial()).getContexts();
            if (targetContexts.isEmpty()) {
                throw new RuntimeException("SNH");
    ...
    private List<TacPlace> getUsedPlaces(CfgNode cfgNodeX, TacPlace victim,
        ...
        if (cfgNodeX instanceof CfgNodeAssignArray) {
            ...
            if (index == -1) {
                ...
                log.error("victim: " + victim);
                throw new RuntimeException("SNH: Could not find formal parameter: " + victim);
            }
...
//  ********************************************************************************
    ...
    public boolean isTree() {
        ...
        log.debug("nodes size: " + this.nodes.size());
        log.debug("edges size: " + edgeCount);
...
//  ********************************************************************************
    ...
    public String makeDotUnique(String graphName) {
        try {
            ...
            log.error("IOError in makeDotUnique",e);
            return "";
        }
    }
    ...
    public void dumpDotUnique(String graphName, String path) {
        try {
            ...
            log.error("Error writing to file " +  path + "/" + graphName + ".dot" ,e);
        }
    }
