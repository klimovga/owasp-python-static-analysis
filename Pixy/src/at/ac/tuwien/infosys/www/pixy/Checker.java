...
public final class Checker {
    ...
    protected static Logger log = Logger.getLogger(Checker.class.getName());
    ...
    // Analyses
    public AliasAnalysis aliasAnalysis;
    public LiteralAnalysis literalAnalysis;
    ...
    public static void main(String[] args) {
        ...
        */
        ...
        if (!MyOptions.optionW) {
            if (MyOptions.optionB) {
                log.debug("File: " + Utils.basename(fileName));
            } else {
        ...
        */
        ...
        if (!MyOptions.optionB) {
            ...
            log.info("Total Time: " + diffTime + " seconds\n\n");
        }
...
//  ********************************************************************************
    ...
    private void readConfig() {
        ...
        String configPath = MyOptions.configDir + "/config.txt";
//        File configFile = new File(configPath);
        Properties props = new Properties();
        try {
//            configPath = configFile.getCanonicalPath();
            InputStream in = Checker.class.getClassLoader().getResourceAsStream(configPath);
            props.load(in);
            in.close();
        } catch (Exception e) {
            log.error("Error reading config file '" + configPath + "'",e);
            Utils.bail("Can't read configuration file: " + configPath);
        }
        ...
        if (includePath != null) {
            ...
            while (tokenizer.hasMoreTokens()) {
                ...
                if (pathElementFile.isDirectory()) {
                    ...
                    log.warn("Warning: Invalid PHP path directory in config file: " + pathElement);
                }
            }
        }
        ...
        if (phpBin == null) {
            ...
            if (!(new File(phpBin)).canRead()) {
                log.warn("Warning: Invalid path to PHP binary in config file: " + phpBin);
                MyOptions.phpBin = null;
            } else {
        ...
        String hsvPath = MyOptions.configDir + "/harmless_server_vars.txt";
//        File hsvFile = new File(hsvPath);
        Properties hsvProps = new Properties();
        try {
//            hsvPath = hsvFile.getCanonicalPath();
            InputStream in = Checker.class.getClassLoader().getResourceAsStream(hsvPath);
            hsvProps.load(in);
            in.close();
...
//  initialize *********************************************************************
    ...
    public ProgramConverter initialize() {
        return initialize(new ProgramConverter(false, MyOptions.optionA));
    }

    public ProgramConverter initialize(ProgramConverter pcv) {
        ...
        */
        ...
        // convert the program
        pcv.setSpecialNodes(this.specialNodes);
/*
        ProgramConverter pcv = new ProgramConverter(
                this.specialNodes, MyOptions.option_A);
*/
        ...
        if (MyOptions.optionL) {
            if (tac.hasEmptyMain()) {
                log.error(MyOptions.entryFile.getPath() + ": library!");
            } else {
                log.error(MyOptions.entryFile.getPath() + ": entry point!");
            }
            System.exit(0);
        }
        ...
        if (MyOptions.optionM) {
            log.info("Maximum number of temporaries: " + tac.getMaxTempId());
        }
...
//  analyzeAliases *****************************************************************
    ...
    AliasAnalysis analyzeAliases(TacConverter tac, boolean cleanup) {
        ...
        ////Checker.report();
        //log.info("Initializing alias analysis");
...
//        Checker.report();
        if (cleanup) {
            log.info("Performing alias analysis: ending clean up");
            this.aliasAnalysis.clean();
        }
        //Checker.report();
        log.info("Performing alias analysis: finished.");
...
//  analyzeLiterals ****************************************************************
    ...
    LiteralAnalysis analyzeLiterals(TacConverter tac) {
        ...
        //Checker.report();
        log.info("Initializing literal analysis");
        this.literalAnalysis = 
            new LiteralAnalysis(tac, this.aliasAnalysis, 
                    new CSAnalysis(this.connectorComp), this.workList);
        //Checker.report();
        log.info("Performing literal analysis");
        this.literalAnalysis.analyze();
        //Checker.report();
        log.info("Performing literal analysis: ending clean up");
        this.literalAnalysis.clean();
        //Checker.report();
        log.info("Performing literal analysis: finished.");
...
//  ********************************************************************************
    ...
    public void analyzeTaint(TacConverter tac, boolean functional) {
        ...
        if (functional) {
            log.info("Performing functional analysis");
            ...
            if (MyOptions.optionV) {
                log.info("Performing call-string analysis!");
            }
        ...
        */
        ...
        /*
        log.info("Performing taint analysis: ending clean up");
        this.taintAnalysis.clean();
        */

        log.info("Performing taint analysis: finished.");
...
//  analyzeIncDom ******************************************************************
    ...
    IncDomAnalysis analyzeIncDom(TacFunction function) {
        ...
        log.info("Initializing incdom analysis");
        this.incDomAnalysis = new IncDomAnalysis(function);
        log.info("Performing incdom analysis");
