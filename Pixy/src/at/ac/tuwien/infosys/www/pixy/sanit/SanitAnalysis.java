...
public abstract class SanitAnalysis extends DepClient {
    ...
    protected static Logger log = Logger.getLogger(SanitAnalysis.class.getName());
...
//  ********************************************************************************
    ...
    public List<Integer> detectVulns(DepClient depClient) {
        ...
        log.debug(name.toUpperCase() + " Sanit Analysis BEGIN");
        ...
        for (DepGraph depGraph : vulnDepGraphs) {
                ...
                if (customSanitNodes.isEmpty()) {
                    ...
                    log.debug("No Sanitization!" +
                            "- " + cfgNode.getLoc() + "- Graphs: "+name+"sanit" + graphcount);
                    sure_vuln_1++;
                    no_sanit++;
                    continue;
                }
                ...
                if (!intersection.isEmpty()) {
                    ...
                    if (ineffBorder != 0) {
                        ...
                        log.debug("Ineffective Sanitization!" +
                                "- " + cfgNode.getLoc() +
                                "- Graphs: "+name+"sanit" + graphcount);
                        possible_vuln++;
        ...
        System.out.println();
        log.debug(name.toUpperCase() + " Sanit Analysis END");
...
//  ********************************************************************************
    ...
    private final FSAAutomaton makeAutoForOp(DepGraphOpNode node, Map<DepGraphNode,FSAAutomaton> deco,
        ...
        // CATCH-ALL ********************************************
            ...
            log.warn("Unmodeled builtin function (SQL-Sanit): " + opName);
